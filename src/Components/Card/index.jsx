...
const Card = ({ data }) => {
  ...
  const removeProductsToCart = (event, productData) => {
    event.stopPropagation()
    context.setCount(context.count - 1)
    const newCartProducts = context.cartProducts.filter(product => product.id !== productData.id)
    ...
    if (isInCart) {
      return (
        <div
          ...
          onClick={(event) => removeProductsToCart(event, data)}>
  ...
  return (
    <div
      ...
      <p className='flex justify-between items-center'>
        ...
        <span className='text-lg font-medium min-w-[25%]'>{formatToDollars(data.price)}</span>
      </p>
    </div>
  )
}
